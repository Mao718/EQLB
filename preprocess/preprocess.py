...
def average_conv_layer(imgs:'(batch,width,width,colors)'
                       ,width:int =3,stride=1,opt='flatten'):
    if opt=='flatten':
        number_of_pixel=width*width*imgs.shape[3]
        conv_kernal=np.ones((width,width,imgs.shape[3],1))
        return tf.nn.conv2d(imgs,conv_kernal,[1,stride,stride,1],padding='VALID') 
